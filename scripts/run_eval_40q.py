#!/usr/bin/env python3
"""Run the full 40-question benchmark against the patched pipeline.

Usage:
    TOPO_LLM_INTENT_MODE=llm_only CORE_PRESTEP_PARALLEL=1 \
        python scripts/run_eval_40q.py
"""
...
BASELINE = ROOT / "scripts" / "openai_services_eval_best_current_fp0_fn2.json"
OUT      = ROOT / "scripts" / "openai_services_eval_patched.json"

def main():
    baseline = json.loads(BASELINE.read_text())
    questions = [(r["idx"], r["question"], r["gold_label"]) for r in baseline["rows"]]
    ...
    print(f"  Baseline:  95.0%  (TP=18 TN=20 FP=0 FN=2)")
